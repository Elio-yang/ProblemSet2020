...
int main()
{
        ...
        for (int i = 0; i < m; i++) {
                ...
                //无记忆或者hash冲突
                if (hash_table[Hash] == NULL || strcmp(hash_table[Hash]->prefix, str) != 0) {
                        hnode *tmp;
                        if(hash_table[Hash] == NULL){
                                hash_table[Hash] = (hnode *) malloc(sizeof(hnode));
                                tmp=hash_table[Hash];
                        }else{
                                tmp=hash_table[Hash];
                                while (tmp->next!= NULL){
