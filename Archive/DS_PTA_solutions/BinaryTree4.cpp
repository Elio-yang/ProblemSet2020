...
T公司的员工层级关系可以表示成一棵树，员工X是员工Y的直接领导，则在树中X是Y的父结点。公司拟组织一场运动会，
 但为了避免尴尬，每个员工都不想与自己的直接领导一起参赛。假定每个员工都对应一个权重（领导的权重不一定比下属大），
 请你编写程序，邀请若干员工参赛，使得参赛人员的总权重和最大。

### 输入格式:

第一行一个正整数n，表示公司的员工人数，员工编号为1...n，n不超过3000。 接下来n行，每行1个整数，
 表示每个员工的权重，值域为[−27,27)。 接下来n-1行，每行为两个空格间隔的整数X和Y，表示Y是X的直接领导。
